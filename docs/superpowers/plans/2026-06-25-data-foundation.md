...
> **For agentic workers:** REQUIRED SUB-SKILL: 使用 `superpowers:subagent-driven-development`（推荐）或 `superpowers:executing-plans` 逐任务执行本计划。步骤使用复选框（`- [ ]`）语法进行跟踪。
...
- [ ] **Step 1: 创建 `pyproject.toml`**（fastapi/uvicorn/pydantic>=2.5/sqlmodel>=0.0.16/sqlalchemy/pyyaml + dev pytest/httpx；pytest 配置 pythonpath=["."]）
- [ ] **Step 2: 创建 `app/config.py`**（Settings 类 + PROJECT_ROOT/DATA_DIR/SEED_DIR/DB_PATH 常量 + db_url 属性）
- [ ] **Step 3: 创建所有空 `__init__.py`**（app/ 及子包、tests/）
- [ ] **Step 4: 安装并验证导入** — Run `pip install -e ".[dev]"`；`python -c "from app.config import settings; print(settings.db_url)"` 无报错
- [ ] **Step 5: 验证 pytest 可运行** — Run `pytest -q` 退出码 0/5 无收集错误
- [ ] **Step 6: Commit** — `chore: 搭建 data-foundation 包结构与配置`
...
- [ ] **Step 1: 写失败测试** — 合法构造、confidence>1 拒绝、CostBand 合法/倒挂拒绝
- [ ] **Step 2: 运行确认失败** — `pytest tests/models/test_base.py -q` ModuleNotFoundError
- [ ] **Step 3: 写实现** — BaseModel + field_validator(confidence) + model_validator(CostBand low≤high)
- [ ] **Step 4: 运行确认通过** — 4 测试 PASS
- [ ] **Step 5: Commit** — `feat(models): SourcedRecord 基类与 CostBand 区间校验`
...
- [ ] **Step 1: 写失败测试** — 传统理综、新高考 6 科、缺 province 拒绝、非法 risk_preference 拒绝、minor_language 可选
- [ ] **Step 2: 运行确认失败**
- [ ] **Step 3: 写实现** — RiskPreference(str,Enum)/MinorLanguage/FamilyResources(6 bool)/StudentProfile(SourcedRecord)
- [ ] **Step 4: 运行确认通过** — 5 测试 PASS
- [ ] **Step 5: Commit** — `feat(models): StudentProfile 考生画像（6 家庭资源布尔位）`
...
- [ ] **Step 1: 写失败测试** — 合法、SalaryBand 倒挂拒绝、stability>1 拒绝、非法 establishment 拒绝
- [ ] **Step 2-4: 失败→实现→通过**
- [ ] **Step 5: Commit** — `feat(models): Career 职业模型（SalaryBand 三档区间校验）`
...
- [ ] **Step 1: 写失败测试** — RatePair 合法/越界拒绝、SocialInsurance 合计、基准单位比例合计 0.2~0.6
- [ ] **Step 2-4: 失败→实现→通过**
- [ ] **Step 5: Commit** — `feat(models): SocialInsurance 五险一金模型 + 全国基准比例`
...
- [ ] **Step 1: 写失败测试** — 合法、rent 倒挂拒绝
- [ ] **Step 2-4: 失败→实现→通过**
- [ ] **Step 5: Commit** — `feat(models): CityCost 城市成本模型（租房三档区间）`
...
- [ ] **Step 1: 写失败测试** — 合法、非法 tier 拒绝、低 confidence 就业率存疑标注
- [ ] **Step 2-4: 失败→实现→通过**
- [ ] **Step 5: Commit** — `feat(models): University 大学三维模型`
...
- [ ] **Step 1: 写失败测试** — 合法、缺 p75 拒绝、avg 字段被拒
- [ ] **Step 2-4: 失败→实现→通过**
- [ ] **Step 5: Commit** — `feat(models): Major 专业模型（强制 P25/P50/P75 三分位）`
...
- [ ] **Step 1: 写失败测试** — 合法、缺 province/year 拒绝、新高考选科自由字符串
- [ ] **Step 2-4: 失败→实现→通过**
- [ ] **Step 5: Commit** — `feat(models): AdmissionRecord 录取 schema`
...
- [ ] **Step 1: 写失败测试** — 3%档(2920→87.6)、边界3000归3%档、20%档(21400→2870)、零/负→0、企业成本公式、到手为正且等于公式、专项扣除使到手更高、7档存在
- [ ] **Step 2-4: 失败→实现→通过** — 8 测试 PASS
- [ ] **Step 5: Commit** — `feat(engine): 五险一金企业成本/到手工资 + 2024 七档累进个税`
...
- [ ] **Step 1: 写失败测试** — 年成本 = 12×((low+high)/2)+学费+住宿
- [ ] **Step 2-4: 失败→实现→通过**
- [ ] **Step 5: Commit** — `feat(engine): 城市成本年化计算`
...
- [ ] **Step 1: 写失败测试** — init_db 建表、插入 CareerRow 取得 id
- [ ] **Step 2-4: 失败→实现→通过** — 含完整 Table 定义
- [ ] **Step 5: Commit** — `feat(db): SQLite 引擎 + 六类 Table 拍平模型 + Session`
...
- [ ] **Step 1: 写失败测试** — career 往返 roundtrip（to_row→to_domain 字段不丢，含 source）
- [ ] **Step 2-4: 失败→实现→通过**（career 完整 + 其余实体同模式）
- [ ] **Step 5: Commit** — `feat(repositories): 六类实体 Table↔Domain 映射层`
...
- [ ] **Step 1: 写失败测试** — 幂等（二次加载数量不变）、缺 source 记录被拒（SeedLoadError）
- [ ] **Step 2-4: 失败→实现→通过** — **移除 placeholder 行**，只实现 load_all_seeds 主入口
- [ ] **Step 5: Commit** — `feat(loader): YAML 种子加载（Domain 校验 + 幂等 upsert + 拒绝损坏记录）`
...
- [ ] **Step 1: 写加载校验测试** — 规模达标 + 每条有来源 + 手编 confidence≤0.6 + note 含待爬虫校准
- [ ] **Step 2: 运行确认失败**
- [ ] **Step 3: 编写种子 YAML** — 量级对齐文档范例（深圳一房一厅 1800、综合月成本 3500-6000）
- [ ] **Step 4: 运行确认通过**
- [ ] **Step 5: Commit** — `feat(data): 六类种子 YAML（合规标注 + 文档量级）`
...
- [ ] **Step 1: 写 conftest + 失败测试** — health、careers 含 source、career 详情、tier 筛选、insurance compute、admissions 空不报错、city cost
- [ ] **Step 2: 运行确认失败**
- [ ] **Step 3: 写 routers** — 每实体 router + main.py lifespan
- [ ] **Step 4: 修复 get_session 为 FastAPI 依赖** — 新增 get_session_dep generator，router 用 Depends(get_session_dep)
- [ ] **Step 5: 运行确认通过** — 7 测试 PASS
- [ ] **Step 6: Commit** — `feat(api): FastAPI 六实体查询端点 + 五险一金计算 + 启动自动加载`
...
- [ ] **Step 1: 写集成测试** — 种子 source/confidence 经 API 返回不丢；手编种子 confidence≤0.6 且 note 含待爬虫校准
- [ ] **Step 2: 运行确认通过**
- [ ] **Step 3: Commit** — `test: 来源追溯全链路集成测试`
...
- [ ] **Step 1: 写合规测试** — 无"保证就业/预测未来收入"措辞；种子含待爬虫校准
- [ ] **Step 2: 写 README** — 安装/加载/启动/测试/架构 + 声明"基于历史数据模拟"
- [ ] **Step 3: 端到端验证** — insurance/compute(9000,全国基准)企业成本≈12060；深圳成本对齐范例
- [ ] **Step 4: 运行合规测试通过**
- [ ] **Step 5: 全量回归** — `pytest -q` 全 PASS
- [ ] **Step 6: Commit** — `docs: README + 端到端验证 + 合规检查`
