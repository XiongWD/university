# DEPRECATED: 旧版多方案优化器模型已废弃，见 volunteer-recommendation-refocus；模型保留以维持 deprecated 端点序列化
"""旧版多方案建议模型（历史名 life_path，V2.2 Phase 4——多方案+家庭预算约束）。

顶层：稳健/均衡/进取三个备选方案（回答"读什么、花多少、走哪里、风险参考"）
第二层：每个方案嵌套冲/稳/保学校清单（回答"这所校录取概率"）
命名不冲突：方案用稳健/均衡/进取，录取用冲/稳/保。
"""
