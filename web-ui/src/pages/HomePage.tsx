          ...
          {lastFormReq && (
            <AIAnalysisPanel
              ...
              streamFactory={(signal) =>
                streamAiRecommend(
                  {
                    score: lastFormReq.total_score,
                    rank: null,
                    track: "历史类",
                    source_province: "河南",
                    primary_subject: "历史",
                    elective_subjects: lastFormReq.elective_subjects ?? [],
                    exam_foreign_language: lastFormReq.exam_foreign_language ?? "英语",
                    subject_scores_detail: {
                      数学: lastFormReq.math_score ?? 0,
                      外语: lastFormReq.foreign_language_score ?? 0,
                    },
                    obey_adjustment: true,
                    extra_preferences: [
                      lastFormReq.prefer_local ? "省内优先" : "",
                      lastFormReq.prefer_public ? "公办优先" : "",
                      lastFormReq.interest_majors?.length
                        ? `兴趣专业：${lastFormReq.interest_majors.join("、")}`
                        : "",
                    ]
                      .filter(Boolean)
                      .join("；"),
